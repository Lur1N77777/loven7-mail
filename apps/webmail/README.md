...
- 地址 JWT 单邮箱登录。
- 单邮箱分享、多邮箱分享、聚合分享。
- 共享链接管理接口：列表、不可逆撤回、过期续期、修改有效期、仅新增邮件模式。
...
| `ASSET_PROXY_DEADLINE_MS` | 否 | 图片/品牌代理总 deadline，可用于故障演练；生产默认采用代码内安全上限。 |

> 后台与用户站分开部署时，请把管理后台的完整 origin 写入 `SHARE_ADMIN_CORS_ORIGINS`。不要设置 `*`；公开分享页正常使用同源相对请求，不需要额外 public CORS。

密钥轮换时，先在 Cloudflare Pages 的 Preview 和 Production 环境分别新增 `SHARE_ENCRYPTION_SECRET_V2`，保留原 `SHARE_ENCRYPTION_SECRET`，再重新部署并验证新旧分享。V2 开启后新分享只用 V2 加密；旧无 `kid` 记录继续用 legacy secret 解密。不要在日志、提交或工单中粘贴任何 secret。
