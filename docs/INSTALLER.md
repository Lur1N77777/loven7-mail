...
3. 在创建前端资源前只读验证 Worker 健康状态、站点密码和管理员口令。
4. 创建或复用两个 Pages 项目。
...
- 新 Worker 模式：一个已经托管在当前 Cloudflare 账号的邮箱域名。安装器会创建 Worker、D1 和前端，但不会代替 Cloudflare Email Routing 的最后确认。
...
- 项目前缀。
- 已托管到 Cloudflare 的完整邮箱域名，例如 `mail.example.net`。
...
```bash
node scripts/installer/cli.mjs --new-worker --domain mail.example.net
...
node scripts/installer/cli.mjs --plan --new-worker --prefix my-mail --domain mail.example.net
```
...
- 同一前缀的邮箱域名或锁定上游提交发生变化时，安装器会先要求确认，避免误改现有 Worker。
...
2. 从外部邮箱发送一封测试邮件，确认 Catch-all 正常投递。
